...
async function findOrCreateCatalogLead(client, catalog, product, body, userId = null) {
  ...
  const created = await client.query(
    `insert into business_manual_leads
     ...
     values ($1, $2, $3, $4, $5, 'Catálogo MG', $6, $7, 'NEW', 'HIGH', $8, $9::jsonb)
     returning *`,
    [
      catalog.business_id,
      userId,
...
function buildWhatsappMessage(catalog, product, lead, body = {}) {
  ...
  const base = template || "Hola, vengo desde Market Games. Me interesa ordenar: {product_name}. Mi nombre es {lead_name}. Vi el catálogo: {catalog_title}. Origen: {origin}.";
  return base
    ...
    .replace(/\{origin\}/g, body.source || body.referral_source || "catalogo Market Games");
}
...
async function syncIntentWithRms(client, intent, lead, phase = "procesamiento", userId = null) {
  ...
  await client.query(
    ...
    [
      ...
      phase === "revenue_generado" ? "Venta marcada desde Catálogos MG" : "Intención de pedido detectada desde Catálogos MG",
      revenuePotential,
      JSON.stringify(meta),
      userId,
    ]
  ...
  return withTransaction(async (client) => {
    ...
    const sale = await client.query(
      `insert into business_sales
       ...
       values ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'Catálogos MG', 'smart_catalog', $10, $11::jsonb)
       returning *`,
      [
        businessId,
...
async function sendPostSaleTicket(businessId, intentId, body = {}, user) {
  ...
  const ticket = await createPostSaleQr(businessId, user, {
    ...
    notes: body.notes || "Ticket postventa generado desde Catálogos MG.",
    ...
    metadata: {
      ...
      ticket_use_case_label: "Postventa Catalogo MG",
      attribution_source: "Catálogos MG",
