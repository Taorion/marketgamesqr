...
const campaignMetricsSelect = `
  select
    ...
    coalesce((select count(*)::int from qr_codes q where q.campaign_id = c.id and coalesce(q.metadata->>'package_ticket_role', '') <> 'final_validable_qr'), 0) as total_qr_generated,
