  ...
  for (const [index, lead] of leadBlueprints.entries()) {
    ...
    await pool.query(
      `insert into attributed_sales
        (business_id, campaign_id, qr_code_id, redemption_id, player_id, sale_amount, purchase_subtotal, currency, sale_confirmed_by_user_id, branch_id, payment_method, product_or_service, notes, created_at, sale_type)
