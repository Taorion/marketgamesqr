(() => {
  ...
  document.addEventListener("click", async (event) => {
    const open = event.target.closest("[data-open-communication-composer]"); const close = event.target.closest("[data-close-communication-composer]"); const pick = event.target.closest("[data-communication-select]"); const all = event.target.closest("[data-communication-select-loaded]"); const send = event.target.closest("[data-send-communication]"); const copy = event.target.closest("[data-copy-communication-social]"); const download = event.target.closest("[data-download-communication-media]"); const removeMedia = event.target.closest("[data-remove-communication-media]"); const clearUrl = event.target.closest("[data-clear-communication-media-url]");
    if (open) { renderOptions(); document.getElementById("communicationComposerForm")?.reset(); setUploadedMedia([]); document.getElementById("communicationComposerModal")?.classList.remove("hidden"); toggleComposer(); return; }
