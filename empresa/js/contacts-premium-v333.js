...
(() => {
  ...
  function renderPreview(data = {}) {
    ...
    ui.previewBody.innerHTML = (data.rows || []).slice(0, 8).map((row) => {
      ...
      return `<tr><td>${row.row_number}</td><td><strong>${escapeHtml([raw.nombre, raw.apellido].filter(Boolean).join(" ") || "—")}</strong><small>${escapeHtml(raw.empresa || "Sin empresa")}</small></td><td>${escapeHtml(raw.numero_documento || raw.correo || raw.telefono || "Sin identificador")}</td><td>${escapeHtml(isCustomer ? `${raw.total_compras || 0} compra(s) · ${raw.valor_acumulado || "0"}` : "Completar después")}</td><td><span class="status-chip ${tone}">${label}</span><small>${escapeHtml(detail)}</small></td></tr>`;
    }).join("");
