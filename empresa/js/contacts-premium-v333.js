...
(() => {
  ...
  function premiumCard(item = {}, segment = "lead") {
    ...
    return `<article class="contact-directory-card-row" role="button" tabindex="0" data-lead-id="${escapeHtml(item.id)}" data-source-type="${escapeHtml(item.source_type || "PLAYER")}" aria-label="Abrir ficha comercial de ${escapeHtml(item.name || "contacto")}">
      ...
      <div class="contact-directory-cell"><strong>${escapeHtml(nextAction)}</strong><small>${escapeHtml(stateLabel)}</small></div>
  ...
  function renderPreview(data={}) { const ui=elements(); ui.counters.innerHTML=[[data.total_rows,"Filas"],[data.valid_rows,"Válidas"],[data.duplicate_rows,"Duplicadas"],[data.invalid_rows,"Con errores"]].map(([value,label])=>`<article><strong>${Number(value||0).toLocaleString("es-CO")}</strong><small>${label}</small></article>`).join(""); ui.counters.classList.remove("hidden"); ui.previewBody.innerHTML=(data.rows||[]).slice(0,8).map((row)=>{const raw=row.data||{};const tone=row.status==="VALID"?"ok":row.status==="DUPLICATE"?"pending":"danger";const label=row.status==="VALID"?"Válida":row.status==="DUPLICATE"?"Duplicada":"Error";return `<tr><td>${row.row_number}</td><td><strong>${escapeHtml([raw.nombre,raw.apellido].filter(Boolean).join(" ")||"—")}</strong><small>${escapeHtml(raw.empresa||"Sin empresa")}</small></td><td>${escapeHtml(raw.numero_documento||raw.correo||raw.telefono||"Sin identificador")}</td><td>${escapeHtml(`${raw.total_compras||0} compra(s) · ${raw.valor_acumulado||"0"}`)}</td><td><span class="status-chip ${tone}">${label}</span><small>${escapeHtml((row.reasons||[]).join(" "))}</small></td></tr>`;}).join(""); ui.previewSection.classList.remove("hidden"); ui.submit.disabled=!Number(data.valid_rows||0); message(data.valid_rows?"Archivo validado. Revisa el resumen antes de importar.":"No hay filas válidas para importar.",data.valid_rows?"success":"error"); }
  async function readFile(file) { const ui=elements(); resetFile(); if(!file)return; if(!/\.csv$/i.test(file.name)){message("Selecciona un archivo con extensión .csv.","error");return;} if(!file.size||file.size>2*1024*1024){message("El CSV debe pesar entre 1 byte y 2 MB.","error");return;} csvState.file=file;csvState.idempotencyKey=crypto.randomUUID();ui.name.textContent=file.name;ui.size.textContent=`${(file.size/1024).toLocaleString("es-CO",{maximumFractionDigits:1})} KB`;ui.summary.classList.remove("hidden");progress(25,"Leyendo archivo...");try{csvState.text=await file.text();progress(60,"Validando encabezados, evidencia y duplicados...");csvState.preview=await api("/api/business/contacts/customers/import-csv/preview",{method:"POST",headers:authHeaders(),body:JSON.stringify({file_name:file.name,file_size:file.size,mime_type:file.type||"text/csv",csv_text:csvState.text})});progress(100,"Validación completada");renderPreview(csvState.preview);setTimeout(()=>progress(0),500);}catch(error){progress(0);message(error.message||"No se pudo validar el CSV.","error");} }
  ...
  async function submit() { const ui=elements();if(!csvState.file||!csvState.preview?.valid_rows||csvState.importing)return;csvState.importing=true;ui.submit.disabled=true;ui.cancel.disabled=true;ui.close.disabled=true;setButtonLoading(ui.submit,true,"Importando...");progress(35,"Creando clientes por lotes de 50...");try{const result=await api("/api/business/contacts/customers/import-csv",{method:"POST",headers:authHeaders(),body:JSON.stringify({file_name:csvState.file.name,file_size:csvState.file.size,mime_type:csvState.file.type||"text/csv",csv_text:csvState.text,idempotency_key:csvState.idempotencyKey})});const batch=result.batch||{};csvState.batchId=batch.id||"";progress(100,"Importación completada");const created=(result.rows||[]).filter((row)=>row.outcome==="CREATED");ui.result.innerHTML=`<strong>Lote procesado</strong><div class="customer-csv-result-grid"><span><strong>${Number(batch.total_rows||0)}</strong><small>filas procesadas</small></span><span><strong>${Number(batch.created_count||0)}</strong><small>clientes creados</small></span><span><strong>${Number(batch.duplicate_count||0)}</strong><small>duplicados omitidos</small></span><span><strong>${Number(batch.error_count||0)}</strong><small>filas con errores</small></span></div>${created.length?`<div class="customer-csv-created-links">${created.slice(0,8).map((row)=>`<button class="text-button" type="button" data-open-imported-customer="${escapeHtml(row.contact_source_id)}" data-source-type="${escapeHtml(row.contact_source_type||"MANUAL")}">Abrir cliente de la fila ${row.row_number}</button>`).join("")}</div>`:""}`;ui.result.classList.remove("hidden");ui.errors.classList.toggle("hidden",!(Number(batch.duplicate_count||0)+Number(batch.error_count||0)));message("La tabla de clientes se actualizó sin recargar la página.","success");state.leadCrmLoaded=false;await premiumLoadLeadCrmData({force:true,quiet:true});renderLeadsView();ui.result.querySelectorAll("[data-open-imported-customer]").forEach((button)=>button.addEventListener("click",()=>{closeCsvModal();openLeadDetail({id:button.dataset.openImportedCustomer,source_type:button.dataset.sourceType||"MANUAL"});}));}catch(error){progress(0);message(error.message||"No se pudo completar la importación.","error");ui.submit.disabled=false;}finally{csvState.importing=false;ui.cancel.disabled=false;ui.close.disabled=false;setButtonLoading(ui.submit,false);ui.submit.disabled=Boolean(csvState.batchId);setTimeout(()=>progress(0),600);} }
