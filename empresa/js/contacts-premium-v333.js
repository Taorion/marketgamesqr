/* global api, apiSafe, authHeaders, bindContactDirectoryControls, businessCommercialOwnerOptions, closeCustomerCsvImportModal,
...
(() => {
  ...
  function premiumRenderDirectory(rows = state.leadCrmRows || []) {
    ...
    board.innerHTML = `<div class="contact-directory-hero">
      ...
      ${audience === "customers" ? '<div class="contact-directory-hero-actions"><button class="solid-button" id="customerCsvImportOpenButton" type="button"><span class="material-symbols-outlined" aria-hidden="true">upload_file</span>Importar clientes CSV</button></div>' : ""}
  ...
  function openCsvModal() { const ui=elements(); if(!ui.modal)return; csvState.lastFocus=document.activeElement; resetFile(); if(ui.defaultSeller)ui.defaultSeller.innerHTML=businessCommercialOwnerOptions("").replace("Sin responsable asignado","Sin responsable predeterminado"); ui.modal.classList.remove("hidden"); ui.modal.setAttribute("aria-hidden","false"); document.body.classList.add("modal-open"); requestAnimationFrame(()=>ui.card?.focus()); }
  ...
  function csvPayload() { const ui=elements(); return { file_name:csvState.file?.name||"",file_size:csvState.file?.size||0,mime_type:csvState.file?.type||"text/csv",csv_text:csvState.text,default_seller_user_id:ui.defaultSeller?.value||null }; }
  async function previewCurrentFile() { if(!csvState.file||!csvState.text)return; progress(60,"Conciliando responsables comerciales..."); try { csvState.preview=await api("/api/business/contacts/customers/import-csv/preview",{method:"POST",headers:authHeaders(),body:JSON.stringify(csvPayload())}); progress(100,"Validación completada"); renderPreview(csvState.preview); setTimeout(()=>progress(0),500); } catch(error) { progress(0); message(error.message||"No se pudo validar el CSV.","error"); } }
  async function readFile(file) { const ui=elements(); resetFile(); if(!file)return; if(!/\.csv$/i.test(file.name)){message("Selecciona un archivo con extensión .csv.","error");return;} if(!file.size||file.size>2*1024*1024){message("El CSV debe pesar entre 1 byte y 2 MB.","error");return;} csvState.file=file;csvState.idempotencyKey=crypto.randomUUID();ui.name.textContent=file.name;ui.size.textContent=`${(file.size/1024).toLocaleString("es-CO",{maximumFractionDigits:1})} KB`;ui.summary.classList.remove("hidden");progress(25,"Leyendo archivo...");try{csvState.text=await readCsvText(file);await previewCurrentFile();}catch(error){progress(0);message(error.message||"No se pudo validar el CSV.","error");} }
  ...
  async function submit() {
    ...
    try {
      const result = await api("/api/business/contacts/customers/import-csv", { method: "POST", headers: authHeaders(), body: JSON.stringify({ ...csvPayload(), idempotency_key: csvState.idempotencyKey }) });
