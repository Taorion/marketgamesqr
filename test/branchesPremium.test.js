...
test("Sedes premium assets are versioned and loaded after the portal final layers", () => {
  ...
  const branchesLayer = html.indexOf("css/branches-premium.css?v=branches-premium-v5-20260822");
  assert.ok(cleanLayer >= 0 && branchesLayer > cleanLayer);
  assert.match(app, /empresa-20260822-activation-calculator-branches-premium-v325/);
  assert.match(css, /#branchFormPanel#branchFormPanel[\s\S]*display:\s*grid\s*!important/);
  assert.match(css, /#branchDetailModal#branchDetailModal[\s\S]*display:\s*grid\s*!important/);
});
